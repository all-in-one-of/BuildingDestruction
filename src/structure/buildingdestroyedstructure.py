...
class BuildingDestroyedStructure():
    ...
    Ideally this class has to be a subclass of a general building structure class,
    but given that building structure class is already created at the moment of the
    creation of this class and we need this class to create all the extra structure
    we make a wrapper and modify only the part we need to destroy the structure
    of the original building.
    We don't need the creatin part, only the structure created. So a wrapper now
    ...
    def destroy_floors(self, crack_type):
        '''
        We only save the destroyed floors, not the full ones.
        when we display_on the floors we will 
        '''
        full_floors_structure = self.full_building_structure.get_floor_structure()
        destroyed_floors = destroyfloorstructure.destroy_floors(full_floors_structure,
                                                                crack_type)
    ...
    def display_off(self):
        '''
        We have to delete the floors from normal structure and for the destroyed one
        '''

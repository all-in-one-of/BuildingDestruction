...
class BuildingStructureContainer(object):
    ...
    def add_default_structures(self, crack, path, inserts, geo, user_restriction_parms):
        full_building_extra_structure = buildingstructure.BuildingStructure(crack, path, inserts,
    ...
    def add_destroyed_structure(self):
        assert('Full building' in self.structures), "No full building structure created"
        destroyed_building_extra_structure = buildingdestroyedstructure.BuildingDestroyedStructure(self.structures['Full building'])
        self.add_structure('Destroyed building', destroyed_building_extra_structure)
    ...
    def display_structure(self, structure_alias):
        ...
        if(self.current_displaying):
            self.structures[current_displaying].display_off()
        self.current_displaying = None
